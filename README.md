...
- Realizar **finetuning a un modelo más reciente** para instrucciones en español: Finalmente intentamos generar textos con aug-pe[^4] en base a un finetune de intrucciones en español para un Llama 2, podriamos realizar un fintune sobre intrucciones en español para modelos mas actuales como algun Llama 3.
- Probar la generación de datos con **modelos cerrados**: Solo adaptamos la parte de modelos abiertos de aug-pe[^4] , podríamos probar también adaptar el mecanismo de generación en base a modelos cerrdos, como GPT 3.5. capaz de tomar mejores intrucciones por ejemplo. 
...
- **Evaluación downstream intermedia en BERT** : Como dijimos en la evaluación, basándonos en los papers[^1] [^2], finetunear un modelo BERT sobre los datos sintéticos y evaluar la accuracy en la predicción de la próxima palabra.
